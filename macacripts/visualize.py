...
def display_instances(image, boxes, captions, masks, ids, scores, title='', ax=None, colors=None, figsize=(16, 16),
                      show_captions=True, show_masks=True, N=None, gray=False):
    if not ax:
    ...
    if show_masks:
        if gray:
            masked_image = color_splash(image, masks)
        else:
    ...
    if show_captions:
        caption_colors = random_colors(N)
        for i in range(N):
            index = ix[i]
            ...
            ax.text(x1 + 3, y1 + 5, caption,
                    color=color, fontstyle='normal', weight='bold', size=12)
